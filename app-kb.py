...
if 'llm_chain' not in st.session_state:
    if (len(sys.argv) > 1):
        allowedLLMs = ["claude_haiku"]
        if sys.argv[1] not in allowedLLMs:
            raise Exception("Unsupported LLM: ", sys.argv[1])
        
        st.session_state['llm_app'] = bchat
        st.session_state['llm_chain'] = bchat.build_chain(sys.argv[1])
    else:
...
def write_top_bar():
    ...
    with col2:
        ...
        header = f"An AI App powered by {provider}!"
...
def handle_input():
    ...
    if 'source_documents' in result:
        ...
        for d in result['source_documents']:
            ...
            s = d.metadata['location']['s3Location']['uri']
            if not (s in document_list):
